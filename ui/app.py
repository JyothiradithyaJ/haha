...
if question:
    planner = st.session_state.get("planner_preview")
    try:
...
if report:
    ...
    with tabs[1]:
        if report.comparative:
            if report.comparison_table: st.dataframe(report.comparison_table, use_container_width=True, hide_index=True)
            else: st.warning("Insufficient evidence retrieved for a fully evidenced comparison table.")
        else: st.info("Comparison mode is shown only when the query explicitly requests comparison/distinction.")
    with tabs[2]:
        st.dataframe([e.model_dump() for e in report.evidence_records], use_container_width=True, hide_index=True)
    with tabs[3]:
        st.subheader("Research gaps")
        for gap in report.research_gaps: st.markdown(f"**{gap.gap_id} — {gap.category.value}**\n\n{gap.description}")
        st.subheader("Limitations")
        for item in report.limitations_in_existing_research: st.markdown(f"- {item}")
    with tabs[4]:
        md_path=st.session_state["md_path"]
        st.markdown(Path(md_path).read_text(encoding="utf-8"))
    with tabs[5]:
        for p in report.papers: st.markdown(f"**[{p.internal_id}]** {p.title} — {p.year or 'N/A'}")

    md_path=Path(st.session_state["md_path"]); json_path=Path(st.session_state["json_path"])
    st.download_button("Download Markdown", md_path.read_bytes(), file_name=md_path.name, mime="text/markdown")
    st.download_button("Download JSON", json_path.read_bytes(), file_name=json_path.name, mime="application/json")
